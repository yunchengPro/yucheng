...
function fileupload_replacefile(id,findstr,replacestr){
    var file = $("#"+id).val();
    if(file.substr(0,1)!=',')
        file=","+file;
...
function fileuploadaction(id,op){
    ...
    .on('fileuploadadd', function (e, data) {
        ...
        $.each(data.files, function (index, file) {
            ...
            if(op.acceptFileTypes.exec("."+filearr[1])==null){
                alert("文件格式有误，请重新上传");
        ...
        if($("#uploadfiles_"+id).find(".filediv_"+id).length+1<=op.maxNumberOfFiles){
            ...
            $.each(data.files, function (index, file) {
                ...
                */
                ...
                //临时
                data.tmp_url = data.file_number+"_"+(Math.random()*100000);
                fileupload_addfile(id,data.tmp_url);
                    ...
                    if(typeof op.getParamUrl != 'undefined' && op.getParamUrl!='' ){
                        ...
                        $.get(op.getParamUrl,function(policy){
                            if(typeof policy =='string' && policy!=''){
                                ...
                                fileupload_replacefile(id,data.tmp_url,multipart_params.key);
                                setTimeout(function(){
                                ...
                                $("<a href='#' class='progress_delete' onclick=\"fileupload_removeupload('file_"+id+"_"+data.file_number+"','"+id+"','"+multipart_params.key+"')\"> 取消 </a>&nbsp;").appendTo('#progress_state_'+data.file_number);
                            }else{
       ...
       // alert("end");
        ...
        if(progress>=100){
            ...
            for(var s in data.files){
                ...
                $('#progress_state_'+data.file_number).css({"font-size":"10px"}).html("<a href='#' class='progress_delete' onclick=\"fileupload_removeupload('file_"+id+"_"+data.file_number+"','"+id+"','"+file.image_url+"')\"> 取消 </a>&nbsp;");
                //$('#progress_state_'+data.file_number).css({"font-size":"10px"});
                $("<span class='progress_ing'>上传中...</span>").appendTo('#progress_state_'+data.file_number);
            }
        }
        ...
        if(data._response.textStatus=="success"){
            ...
            for(var s in data.files){
                ...
                if(op.returnUrl==true){
                    ...
                    if (data.result.url) {
                        ...
                        $("#progress_state_"+data.file_number).find(".progress_ing").remove();
                        $("#progress_state_"+data.file_number).find(".progress_delete").html("删除");
            ...
            if(op.returnUrl==true){
                ...
                //alert(op.imgid);
                $("#"+op.imgid).attr('src',op.domain+fileurl.substr(0,fileurl.length-1));
                //return fileurl.substr(0,fileurl.length-1);
                if(op.uploadfunc!=''){
                    eval(op.uploadfunc+"('"+fileurl.substr(0,fileurl.length-1)+"')")
                }
            }
