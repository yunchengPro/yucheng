...
function selClass($this){
    ...
    $.getJSON('/product/index/ajaxcategory/', {gc_id : data_str.gcid, deep: deep}, function(data) {
        if (data != null) {
...
function disabledButton() {
    ...
    if ($('#class_id').val() != '' && $("#store_id").val() != '' && $("#t_id").val() != '') {
...
$(function(){
    ...
    setInterval("disabledButton()", 1000);


    
    // $("#store_name").mouseleave(function(){
    //     disabledButton();
    // });

    // $("#t_name").mouseleave(function(){
    //     disabledButton();
    // });
    
    // 常用分类选择
    $('#commListArea').find('span[nctype="staple_name"]').on('click',function() {
        $('#dataLoading').show();
        $('.wp_category_list').addClass('blank');
        $this = $(this);
        eval('var data_str = ' + $this.parents('li').attr('data-param'));
        $.getJSON('/product/index/ajaxcategory?gc_id=' + data_str.stapleid, function(data) {
            if (data.done) {
