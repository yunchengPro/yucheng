...
function Hui_admin_tab(obj){
	if($(obj).attr('_href')){
		...
		//选中事件处理 
		var newobj = $(obj).parent().parent().find('a');
		
		var clickObj = $('.menu_dropdown').find('a');
		
		$.each(clickObj,function(n,value) { 
           	$(value).attr("class","");
  		});  
		$.each(newobj,function(n,value) { 
           	$(value).attr("class","");
  		});  
		$(obj).attr("class","clickOn");
		//选中事件处理 END
...
$(function(){
	...
	$(document).on("click","#min_title_list li",function(){
		...
		var spanHref = span.attr('data-href');
		
		$.each(clickObj,function(n,value) { 

			var _href = $(value).attr('_href');
			$(value).attr("class","");
			if(_href == spanHref){
           		$(value).attr("class","clickOn");
           		$("#min_title_list li").removeClass("active").eq(bStopIndex).addClass("active");
				iframe_box.find(".show_iframe").hide().eq(bStopIndex).show();
           	}
