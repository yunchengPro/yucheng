...
function displaynavbar(obj){
	...
	if($(obj).hasClass("open")){
		...
		$menu.find("span").show().prev().css('margin-left','0');
	}else{
		$(obj).addClass("open");
		$("body").addClass("big-page");
		$menu.find("span").hide().prev().css('margin-left','150px');
		$menu.find("dd").hide();

		// //收起来时 点击图标展开菜单
		$menu.on('click','dt',function(){
			$menu.find("span").show().prev().css('margin-left','0');
			$(".dislpayArrow").find("a").removeClass("open");
			$("body").removeClass("big-page");
		});
	}
}
...
// $(function(){
// 	//收起来时 点击图标展开菜单
// 	$("body").on('click','.Hui-icon-btn',function(){
// 		var $menu = $(this).parents('.menu_dropdown');
// 		$menu.find("span").show().prev().css('margin-left','0');
// 		$(".dislpayArrow").find("a").removeClass("open");
// 		$("body").removeClass("big-page");
// 	});
...
$(function(){
    ...
    /*全选 修改 talon 2017-5-11 12:00:04*/
    $(document).on("click" ,"table thead th input:checkbox", function(){
		$(this).closest("table").find("tr > td:first-child input:checkbox").prop("checked",$("table thead th input:checkbox").prop("checked"));
    });
