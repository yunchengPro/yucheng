...
class OrdOrderItemModel {
	...
	public function __construct() {
		$this->_modelObj = Db::Table('OrdOrderItem');
	}
...
// 	}
    ...
    */
    public function getAllItemDetailByOrderNO($data) {
        $itemlist = $this->_modelObj->getList(array("orderno" => $data['orderno']), "businessid,businessname,skuid, productnum, productid,productname,prouctprice,bullamount,skudetail,remark");
    ...
    */
    public function getItemDetailBySkuid($data) {
        ...
        if(!empty($data['skuid'])) {
            ...
            $goods = $specOBJ->getRow(array("id" => $data['skuid']), "id,productname,prouctprice,bullamount,spec,productimage as thumb");
            ...
            if(empty($goods)){
	            ...
	            $goods = $proOBJ->getRow(array("id" => $data['productid']), "id,productname,prouctprice,bullamount,thumb");
	            $goods['skudetail'] = '';
	        }
	       // print_r($goods);
    ...
    */
    public function getItemPrice($params) {
        $itemInfo = $this->getRow(array("orderno" => $params['orderno'], "productid" => $params['productid'], "skuid" => $params['skuid']), "prouctprice, bullamount, productnum, enable");
...
//         $returnList = $OrdOrderReturn->getRow(array("order_code"=>$params['orderno'],"productid"=>$params['productid'],"skuid"=>$params['skuid'],"orderstatus"=>array(array("=",4), array("=",12,"or"), array("=",14,"or"))), "SUM(productnum) as productnum ");
        $returnList = $OrdOrderReturn->getRow(array("order_code"=>$params['orderno'],"productid"=>$params['productid'],"skuid"=>$params['skuid'],"orderstatus"=>["in", "4,14"]), "SUM(productnum) as productnum ");
