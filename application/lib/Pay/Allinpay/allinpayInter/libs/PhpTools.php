<?php 
namespace app\lib\Pay\Allinpay\allinpayInter\libs;

use app\lib\Pay\Allinpay\allinpayInter\libs\ArrayAndXml;

class PhpTools{
	 
	
	var $certFile = './data/allinpay-pds.pem';//通联公钥证书
	var $privateKeyFile = './data/20060400000044502.pem';//商户私钥证书
	var $password = '111111';//商户私钥密码以及用户密码
//	const  apiUrl = 'http://172.16.1.11:8080/aipg/ProcessServlet';//通联系统对接请求地址（内网）
	public $arrayXml ;
    var $apiUrl = 'http://113.108.182.3:8083/aipg/ProcessServlet';//通联系统对接请求地址（外网,商户测试时使用）
...
//		echo ($xmlSignSrc);
	...
	public function send($params){
		...
		//$xmlSignPost = iconv("utf-8", "gbk", $xmlSignPost);
		//echo $xmlSignPost;
		//exit;
		...
		//$xmlResponse = iconv("gbk", "utf-8", $xmlResponse);
		//print_r("返回报文如下：");
		//print_r($xmlResponse);
