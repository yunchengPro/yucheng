...
abstract class Validator
{
     ...
     *
     * @code php
     * if (!DealCore_Classes_Validator::validate($value, 'max', 5))
     * {
     *     echo 'value 不能大于 5';
     * }
     * @endcode
     *
     * $validation 参数是验证规则。
     * 如果使用 DealCore_Classes_Validator 自带的验证规则，可以使用如下的写法：
     *
     * @code php
     * DealCore_Classes_Validator::validate($value, 'between', 1, 5);
     * @endcode
     *
     * 'between' 是验证规则的名字，对应于 DealCore_Classes_Validator::validate_between() 方法。
     * 因此要使用 DealCore_Classes_Validator 的其他验证方法，将方法名中的“validate_”去掉即为验证规则名。
     *
     * 如果要使用自定义的验证规则，有三种写法：
     *
     * @code php
     * // 使用某个类的静态方法做验证方法
     * DealCore_Classes_Validator::validate($value, array('MyClass', 'myMethod'), $args);
     *
     * // 功能同上
     * DealCore_Classes_Validator::validate($value, 'MyClass::myMethod', $args);
     *
     * // 使用某个对象的方法做验证方法
     * DealCore_Classes_Validator::validate($value, array($my_obj, 'myMethod), $args);
     * @endcode
     *
     ...
     *
     * @code php
     * $ret = DealCore_Classes_Validator::validateBatch($value, array(
     *         array('is_int'),
     *         array('between', 2, 6),
     * ));
     * @endcode
     ...
     * @code php
     * $failed = null;
     * $ret = DealCore_Classes_Validator::validateBatch($value, $validations, true, $failed);
     *
     ...
     * @code php
     * // validate() 的写法
     * DealCore_Classes_Validator::validate($value, 'max', 6);
     *
     * // validateByArgs() 的写法
     * DealCore_Classes_Validator::validateByArgs('max', array($value, 6));
     * @endcode
     *
    ...
    static function validateByArgs($validation, array $args)
    {
        ...
        if (is_null($internal_funcs))
        {
            $internal_funcs = array('between', 'equal', 'greater_or_equal',
                ...
                'regex', 'same', 'skip_empty', 'skip_null', 'skip_on_failed','is_phone','is_tel','in','is_money','password','name','max_length_utf8');
            $internal_funcs = array_flip($internal_funcs);
        }

        // DealCore_Classes_Validator 类的验证方法
    ...
    static function validate_is_phone($value){
        return preg_match('/^1[3|4|5|8][0-9]\d{8}$/',$value);
    }
    /**
     * 是否是电话号码
     * @param unknown $value
     * @return number
     */
    static function validate_is_tel($value){
     ...
     * @return boolean
     */
    static function validate_is_money($money) {
     ...
     * @return boolean
     */
    static function validate_in($value,$aInArr)
    {
    	return in_array($value,$aInArr);
    }
     ...
     * @return boolean
     */
	static function validate_password($value){
    	$pattern = '/^[0-9|a-z]{6,20}$/i';
    	return preg_match($pattern, $value);
    }
    
    public function validate_name($value){
    	$pattern = '/^([\x{4e00}-\x{9fa5}]{2,7})|([a-z]{1,20})$/ui';
    	return preg_match($pattern, $value);
    }
    
    static function validate_max_length_utf8($value, $len)
    {
    	return mb_strlen($value,'utf8') <= $len;
    }
}