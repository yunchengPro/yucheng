...
use app\lib\Db;
	...
	foreach ($pagelist as $key => $row) {
	    ...
	    $pagelist[$key]['act'][]= array("type"=>"confirm","name"=>"button","value"=>"取消vip", "_width"=>"500","_height"=>"200","_title"=>"是否降为普通牛商","_url"=>"/Customer/BullBus/isvip?id=".Encode($row['id'])."&status=-1");
...
//         "bt1" => array("type" => "popup_listpage", "name" => "button", "value" => "快速添加供应商", "_width" => "550", "_height"=>"450", "_title"=>"快速添加供应商", "_url"=>"/Customer/BullBus/quickAddBullBus"),
            "bt1" => array("type"=>"popup_listpage","name"=>"button","value"=>"添加vip供应商","_width"=>"650","_height"=>"550","_title"=>"添加vip供应商","_url"=>"/Customer/BullBus/addBullBus?type=1"),
    );
?> 
<?php if ($full_page){?>
<!---头部-->
