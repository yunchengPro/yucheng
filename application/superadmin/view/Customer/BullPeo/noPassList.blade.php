...
use app\lib\Db;
    ...
    */
    $list_set = array(
        ...
        "pay_status"=>array("name"=>"支付状态", 'data_arr' => ['1' => '未支付', '2'=>'已支付']),
        "remark"=>array("name"=>"不通过原因"),
...
//        $pagelist[$key]['businessid'] = Db::Model("BusBusiness")->getRow(['id'=>$row['businessid']],'businessname')['businessname'];
        ...
        //$pagelist[$key]['act'][]= array("type"=>"confirm","name"=>"button","value"=>"通过","_width"=>"500","_height"=>"200","_title"=>'是否确定通过?',"_url"=>"/Customer/BullPeo/pass?id=".Encode($row['id']));

        //$pagelist[$key]['act'][]= array("type"=>"popup_listpage","name"=>"button","value"=>"未通过","_width"=>"650","_height"=>"550","_title"=>'未通过原因',"_url"=>"/Customer/BullPeo/noPass?id=".Encode($row['id']));

//         $pagelist[$key]['act'][]= array("type"=>"popup_listpage","name"=>"button","value"=>"编辑","_width"=>"650","_height"=>"550","_title"=>"编辑","_url"=>"/Customer/BullPeo/editBullCus?id=".Encode($row['id']));

//         $pagelist[$key]['act'][]= array("type"=>"confirm","name"=>"button","value"=>"删除","_width"=>"500","_height"=>"200","_title"=>'是否确定删除该记录?',"_url"=>"/Customer/BullPeo/delBullCus?ids=".Encode($row['id']));

        $pagelist[$key]['act'][] = array("type"=>"confirm","name"=>"button","value"=>"还原待审核","_width"=>"500","_height"=>"200","_title"=>"还原待审核?","_url"=>"/Customer/BullPeo/restorePeo?id=".Encode($row['id']));
