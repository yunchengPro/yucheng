<?php
	...
	foreach ($pagelist as $key => $row) {
		if($row['status'] == 1){
		    $pagelist[$key]['act'][] = array("type" => "popup_listpage", "name" => "button", "value" => "通过审核", "_width" => "650", "_height" => "550", "_title" => "通过审核", "_url" => "/Customer/Apply/applymanagerpass?id=".Encode($row['id'])."&status=2");
		    $pagelist[$key]['act'][] = array("type" => "popup_listpage", "name" => "button", "value" => "拒绝通过", "_width" => "650", "_height" => "550", "_title" => "拒绝通过", "_url" => "/Customer/Apply/applymanagerpass?id=".Encode($row['id'])."&status=3");
		}
