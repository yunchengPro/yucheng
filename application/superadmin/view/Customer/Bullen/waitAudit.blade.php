...
//        $pagelist[$key]['businessid'] = Db::Model("BusBusiness")->getRow(['id'=>$row['businessid']],'businessname')['businessname'];
        ...
        $pagelist[$key]['act'][]= array("type"=>"confirm","name"=>"button","value"=>"通过","_width"=>"500","_height"=>"200","_title"=>'是否确定通过?',"_url"=>"/Customer/Bullen/pass?id=".Encode($row['id']));

        $pagelist[$key]['act'][]= array("type"=>"popup_listpage","name"=>"button","value"=>"不通过","_width"=>"650","_height"=>"550","_title"=>'不通过原因',"_url"=>"/Customer/Bullen/noPass?id=".Encode($row['id']));

        $pagelist[$key]['act'][]= array("type"=>"popup_listpage","name"=>"button","value"=>"编辑","_width"=>"650","_height"=>"550","_title"=>"编辑","_url"=>"/Customer/Bullen/editBullenReco?id=".Encode($row['id']));
