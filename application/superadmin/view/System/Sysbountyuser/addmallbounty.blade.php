...
<?php 
    ...
    */
    ...
    $str = '';
    if(!empty($bountymall)){
        $str = '重新设置奖励金后领取金额和已领取金额将清零';
    }

    $field = array(
            "amount"=>array("type"=>"text","name"=>"奖励金额","validator"=>"required:true", "value"=>$bountymall['amount']),
            "disamount"=>array("type"=>"text","name"=>"已领取奖励金额","validator"=>"required:true", "value"=>$bountymall['disamount']),
            "hasamount"=>array("type"=>"text","name"=>"剩余奖励金额","validator"=>"required:true", "value"=>$bountymall['hasamount']),
            "id"=>array("type"=>"hidden","value"=>1),
            "submit"=>array("type"=>"submit","name"=>"提交"), //加该参数表示独立页面，不加该参数表示弹窗
        );
