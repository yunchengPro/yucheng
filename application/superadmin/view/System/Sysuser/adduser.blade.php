...
<?php 
    ...
    */
    ...
    $field = array(
            "username"=>array("type"=>"text","name"=>"登录名称","validator"=>"required:true","value"=>$user['username']),
            "mobile"=>array("type"=>"text","name"=>"手机号码","value"=>$user['mobile']),
            "roleid"=>array("type"=>"select","name"=>"角色","value"=>$user['roleid'],"option"=>$roleOption),
            "userpwd"=>array("type"=>"password","name"=>"密码","validator"=>"required:true","value"=>$user['userpwd']),
            "enable"=>array("type"=>"select","name"=>"启用状态","value"=>$user['role'],'option'=>['0'=>'停用','1'=>'启用']),
            //"submit"=>array("type"=>"submit","name"=>"提交"), //加该参数表示独立页面，不加该参数表示弹窗
        );
