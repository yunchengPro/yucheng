...
<?php 
    ...
    */
    ...
    $field = array(
            ...
            "district_name"=>array("type"=>"text","name"=>"商圈名称","validator"=>"required:true", "value"=>$District['district_name']),
            "areaid"=>array("type"=>"selectpicker", "name"=>"所属区域", 'option' => $city, "value"=>$District['areaid']),
            "sort"=>array("type"=>"text","name"=>"排序","validator"=>"required:true", "value"=>$District['sort']),
            "enable"=>array("type"=>"select", "name"=>"启用状态", 'option' => ['1' => '启用', '-1'=>'禁用'],"validator"=>"required:true", "value"=>$District['enable']),
            "id"=>array("type"=>"hidden","value"=>$District['id']),
            //"submit"=>array("type"=>"submit","name"=>"提交"), //加该参数表示独立页面，不加该参数表示弹窗
        );
