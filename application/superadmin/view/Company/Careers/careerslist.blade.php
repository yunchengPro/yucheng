...
use app\lib\Db;
    ...
    */
    $list_set = array(
        ...
        "id"=>array("name"=>"ID"),
        "position"=>array("name"=>"招聘职位",),
        "position_en"=>array("name"=>"英文介绍"),
        "sort"=>array("name"=>"排序","data_sort"=>true,"_url"=>"/Company/Careers/sortcareers",'sort'=>true),
    ...
    */
    foreach($pagelist as $key=>$row){
        ...
        $pagelist[$key]['act'][]= array("type"=>"newpage","name"=>"button","value"=>"修改","_width"=>"500","_height"=>"400","_title"=>"修改<font color='green'>".$row['position']."</font>","_url"=>"/Company/Careers/editcareers?id=".Encode($row['id']));

       
        
        $pagelist[$key]['act'][]= array("type"=>"confirm","name"=>"button","value"=>"删除","_width"=>"500","_height"=>"400","_title"=>"确定删除?","_url"=>"/Company/Careers/delcareers?ids=".Encode($row['id']));
    ...
    */
    $search = array(
        "position"=>array("type"=>"text","name"=>"职位描述/关键字"),
     
    );

    //自动生成按钮
    $button = array(
            "bt1"=>array("type"=>"newpage","name"=>"button","value"=>"添加招聘","_width"=>"600","_height"=>"450","_title"=>'添加招聘',"_url"=>"/Company/Careers/addcareers"),
