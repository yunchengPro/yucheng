...
<div class="admin">
    <div class="tab">
        <div class="tab-head">
            <ul class="tab-nav">
                <li class="active"><a href="#tab-set"><?php  //=$this->actionname?>运费模板</a></li>
            </ul>
        </div>
        <div class="tab-body">
            <br />
            <div class="tab-panel active" id="tab-set">
                <form method="post" class="form-x" id="transport" action="/setup/transport/save">
                    ...
                    <div class="form-group">
                        ...
                        <div class="field">
                            <div class="button-group button-group-small radio">
                                <?php  //=$this->html("select",["name"=>"business_id","class"=>"input","option"=>$this->business_list,"other"=>"style='height:32px;'", "value"=>$this->info['business_id']])?>
                                <p class="J_Message" style="display: none;" error_type="business_id"><i class="icon-exclamation-sign"></i>必须选择所属商家</p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <div class="field">
                            <div class="button-group button-group-small radio">
                                <input type="text" class="input" size="50" id="title" name="title" value="<?php  //=$this->info['title']?>">
                    ...
                    <div class="form-group">
                        <div class="label">
                            <label><?php  //=_('计价方式')?></label>
                        </div>
                        <div class="field">
                            <div class="button-group button-group-small radio">
                                <input type="radio" name="valuation_type" <?php  //if(empty($this->info['valuation_type']) || ($this->info['valuation_type'] == 1)):?>checked<?php  //endif;?>  value="1" id="j_number"><label for="j_number">&nbsp;按件数&nbsp;</label>
                                <input type="radio" name="valuation_type" <?php  //if($this->info['valuation_type'] == 2):?>checked<?php  //endif;?> value="2" id="j_weight"><label for="j_weight">&nbsp;按重量&nbsp;</label>
                                <input type="radio" name="valuation_type" <?php  //if($this->info['valuation_type'] == 3):?>checked<?php  //endif;?> value="3" id="j_volume"><label for="j_volume">&nbsp;按体积&nbsp;</label>
                        ...
                        <div class="field">
                            <div class="button-group button-group-small radio" id="express">
                                ...
                                <script>
                                    ...
                                    showExpress(<?php  //=$this->info['valuation_type']?>);
                    ...
                    <div id="dialog_areas" class="dialog-areas" style="display:none">
                        <div class="ks-contentbox">
                            ...
                            <ul id="J_CityList">
                                <?php  //php include(ROOT_DIR . DS . 'data'.DS.'area' . DS . 'format_area.html');?>
                            </ul>
                            <div class="bottom">
                    ...
                    <div class="form-button">
                        <?php  //=$this->formtoken?>
                        <input type="hidden" name="transport_id" value="<?php  //=$this->info['id']?>" />
...
<script type="text/javascript">
    ...
    $("#btn_add_clone").click(function(){
        var html = '<tr><td></td><td><input class="input" style="width:50px" type="text" name="at_value[s_'+i+'][sort]" value="0" placeholder="排序" /></td><td><input class="input" style="width:100px" type="text" name="at_value[s_'+i+'][name]" size="50" placeholder="属性名称" /></td><td><input class="input" style="width:350px" type="text" name="at_value[s_'+i+'][value]" size="100" placeholder="属性可选值，用英文逗号隔开" /></td><td><input style="width:20px" type="checkbox" checked value="1"  name="at_value[s_'+i+'][show]" /></td><td><a style="color:blue" class="btns">删除</a></td></tr>';
...
<script>
    $(function(){
        ...
        <?php  //if (is_array($this->transport_extend)):?>

            <?php  //foreach($this->transport_extend as $tk=>$tv):?>

                <?php  //if($tv['is_default'] == 1):?>
                    var cur_tr = $('.tbl-except').prev();

                    $(cur_tr).find('input[data-field="start"]').val('<?php  //php echo $tv['snum'];?>');
                    $(cur_tr).find('input[data-field="postage"]').val('<?php  //php echo $tv['sprice'];?>');
                    $(cur_tr).find('input[data-field="plus"]').val('<?php  //php echo $tv['xnum'];?>');
                    $(cur_tr).find('input[data-field="postageplus"]').val('<?php  //php echo $tv['xprice'];?>');
                ...
                <?php  //else:?>
                    ...
                    $(cur_tr).find('.area-group>p').html('<?php  //php echo $tv['area_name'];?>');

                    $(cur_tr).find('input[type="hidden"]').val('<?php  //php echo trim($tv['area_id'],',');?>|||<?php  //php echo $tv['area_name'];?>');
