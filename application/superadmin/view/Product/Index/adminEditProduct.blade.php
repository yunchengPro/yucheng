...
{include file="Product/Index/admineditstep" /}
                    ...
                    if(tmp_k_s[1]==tmp_s[1]){
                        ...
                        if(flag && (typeof(tmpa_V[k_s])=='undefined' || tmpa_V[k_s]=='' )){
                            ...
                            if(typeof(V[k_s]) != 'undefined' && V[k_s] !=''){
                                ...
                                $(this).attr('src',"//nnhcoupon.oss-cn-shenzhen.aliyuncs.com/"+V[k_s]);
                            }else{
...
function getSpecTable(){
    ...
    if(speclist.length>0){
        ...
        for(var i=0;i<speclist.length;i++){
            ...
            //str+='<td><div class="formControls col-xs-9 col-sm-8"><span name="spec['+spec_bunch+'][image_upload]" id="spec-'+spec_bunch+'-image_upload" class="btn btn-danger fileinput-button"><span>添加图片</span><input type="file" multiple="" name="file"></span>&nbsp;<font>(文件数：1；文件大小：4M)</font><div style="margin-top:15px;" class="uploadfiles" id="uploadfiles_spec-'+spec_bunch+'-image"><input class="text input image" style="width:90px;" type="hidden" name="spec['+spec_bunch+'][image_upload]" id="spec-'+spec_bunch+'-image" data_type="image" nc_type="'+spec_bunch+'|image" value="" /></div><script type="text/javascript">fileupload(\'spec-'+spec_bunch+'-image\',\'{"url":"/uploadfile/jqueryfileupload.php","domain":"//nnhcoupon.oss-cn-shenzhen.aliyuncs.com/","maxFileSize":4194304,"maxNumberOfFiles":"1","savefileurl":"/Sys/upload/getfile","getParamUrl":"/Sys/upload/policy","server_type":"NNH/images","formData":{"server_type":"NNH/images"}}\');<\/script>（建议尺寸:200pxX200px）</div></td>';

            str +='<td><div class=\"formControls col-xs-9 col-sm-8\"><span class=\"btn btn-danger fileinput-button\" id=\"spec-'+spec_bunch+'-image_upload\" name=\"spec['+spec_bunch+'][image_upload]\"><span>添加图片</span><input type=\"file\" name=\"file\" multiple=\"\"></span>&nbsp;<font>(文件数：1,上传多张只会保存最新的；文件大小：4M)</font><div id=\"uploadfiles_spec-'+spec_bunch+'-image\" class=\"uploadfiles\" style=\"margin-top:15px;\"><input type=\"hidden\" id=\"spec-'+spec_bunch+'-image\" name=\"spec['+spec_bunch+'][image]\" nc_type=\"'+spec_bunch+'|image\" value=\"\"><div class=\"filediv_spec-'+spec_bunch+'-image\" id=\"file_spec-'+spec_bunch+'-image_1327\" style=\"float: left; width: 100px; margin-right: 10px;\"><div><img id=\"spec-'+spec_bunch+'-image_show\" nc_type=\"'+spec_bunch+'|image\" src=\"\" width=\"100px\"></div><div style=\"text-align:center;\"></div><span id=\"url\" nc_type=\"'+spec_bunch+'|image\" style=\"display:none\"></span></div></div><script type=\"text/javascript\">fileupload(\'spec-'+spec_bunch+'-image\',\'{\"url\":\"/uploadfile/jqueryfileupload.php\",\"domain\":\"//nnhcoupon.oss-cn-shenzhen.aliyuncs.com/\",\"maxFileSize\":4194304,\"maxNumberOfFiles\":\"4\",\"savefileurl\":\"/Sys/upload/getfile\",\"getParamUrl\":\"/Sys/upload/policy\",\"server_type\":\"NNH/images\",\"formData\":{\"server_type\":\"NNH/images\"}}\');<\/script>（建议尺寸:200pxX200px）</div></td></tr>';
