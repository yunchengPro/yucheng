...
//         "comamount"=>array("name"=>"企业金额"),
        "poundage"=>array("name"=>"手续费"),
        "need_money"=>array("name"=>"所需支付金额"),
        "pay_money"=>array("name"=>"实际支付金额"),
        "pay_time"=>array("name"=>"支付时间"),
        "handle_user"=>array("name"=>"处理人"),
        "status_str"=>array("name"=>"处理状态"),
        "act"=>array("name"=>"操作"),
    );
...
//         $pagelist[$key]['act'][] = array("type"=>"newpage","name"=>"button","name"=>"button","value"=>"查看交易明细","_width"=>"500","_height"=>"200","_title"=>"查看交易明细","_url"=>"/Amount/Index/balance?mobile=".Encode($row['customer_mobile']));

        if($pagelist[$key]['status'] == 0){
            $pagelist[$key]['act'][]= array("type"=>"popup_listpage","name"=>"button","value"=>"通过","_width"=>"500","_height"=>"400","_title"=>'实际支付金额',"_url"=>"/Amount/Withdrawals/pass?id=".Encode($row['id']));
        }
        if($pagelist[$key]['status'] == 0 || $pagelist[$key]['status'] == 4){
