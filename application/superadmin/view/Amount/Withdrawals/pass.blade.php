...
<?php 
    ...
    */
    ...
    $field = array(
            // "total" => array("type"=>"text", "name" => "已提现金额", "data"=>$info['total']),
            "amount" => array("type" => "text", "name" => "本次提现金额", "data" => DePrice($info['amount'])),
            "poundage" => array("type" => "text", "name" => "手续费", "data" => DePrice($info['poundage'])),
            "pay_money"=>array("type"=>"text","name"=>"实际支付金额","validator"=>"required:true","value"=>$info['pay_money'],"other"=>"readonly='true'"),
            "id"=>array("type"=>"hidden","value"=>$info['id']),
            //"submit"=>array("type"=>"submit","name"=>"提交"), //加该参数表示独立页面，不加该参数表示弹窗
        );
