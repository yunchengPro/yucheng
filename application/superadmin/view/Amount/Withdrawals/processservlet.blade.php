    ...
    */
    foreach($pagelist as $key=>$row){
        ...
        if($pagelist[$key]['status'] == 0)
            $pagelist[$key]['act'][]= array("type"=>"confirm","name"=>"button","value"=>"支付","_width"=>"500","_height"=>"400","_title"=>'是否确定支付金额['.$row['amount'].']?注意等待响应过程可能会延时1秒左右，请不要重复再操作！',"_url"=>"/Amount/Withdrawals/passprocessservlet?id=".Encode($row['id']));
