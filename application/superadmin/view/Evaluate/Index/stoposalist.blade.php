<?php
    ...
    */
    foreach($pagelist as $key=>$row){
        ...
        $pagelist[$key]['act'][]= array("type"=>"confirm","name"=>"button","value"=>"删除","_width"=>"500","_height"=>"200","_title"=>'是否确定删除该记录?',"_url"=>"/Evaluate/Index/delposa?ids=".Encode($row['id']));
    ...
    */
    ...
    //自动生成按钮
    $button = array(
         "bt2"=>array("type"=>"confirm_all","name"=>"button","value"=>"批量删除","_width"=>"500","_height"=>"200","_title"=>'删除选中项?',"_url"=>"/Evaluate/Index/delposa?rel=delete")
        );
?> 
<?php if ($full_page){?>
<!---头部-->
