...
<?php 
    ...
    */
    ...
    $field = array(
            //"type"=>array("type"=>"select", "name"=>"类型", 'option' => ['1' => '商城首页', '2'=>'现金专区', '3'=>'现金加牛币专区'], "validator"=>"required:true", "value"=>$bannerData['type']),
            "bname"=>array("type"=>"text","name"=>"推荐名称","validator"=>"required:true", "value"=>$moduleBannerData['bname']),
            "thumb"=>array("type"=>"imgupload", "name"=>"图片地址","validator"=>"required:true", "value"=>$moduleBannerData['thumb']),
            "urltype"=>array("type"=>"select", "name"=>"跳转类型", 'option' => $skipurl,"validator"=>"required:true", "value"=>$moduleBannerData['urltype']),
            "url"=>array("type"=>"text","name"=>"跳转链接","validator"=>"required:true", "value"=>$moduleBannerData['url']),
            "sort"=>array("type"=>"text","name"=>"排序","validator"=>"required:true", "value"=>$moduleBannerData['sort']),
            "moduleid"=>array("type"=>"hidden","value"=>$moduleId),
            "id"=>array("type"=>"hidden","value"=>$moduleBannerData['id']),
            "submit"=>array("type"=>"submit","name"=>"提交"), //加该参数表示独立页面，不加该参数表示弹窗
        );
