...
use app\lib\Db;
    ...
    */
    $list_set = array(
        ...
        "type"=>array("name"=>"类型", 'data_arr' => ['1' => '头部', '2'=>'中间', '3'=>'底部']),
        "city_id"=>array("name"=>"城市编码"),
        "city"=>array("name"=>"所属城市"),
    ...
    */
    foreach($pagelist as $key=>$row){
        if(empty($row['city']))
            $pagelist[$key]['city'] = $city[$row['city_id']];
        //$pagelist[$key]['businessid'] = Db::Model("BusBusiness")->getRow(['id'=>$row['businessid']],'businessname')['businessname'];

        $pagelist[$key]['act'][]= array("type"=>"newpage","name"=>"button","value"=>"编辑","_width"=>"550","_height"=>"450","_title"=>"编辑","_url"=>"/Mall/StoBanner/editStoBanner?id=".Encode($row['id']));

        $pagelist[$key]['act'][]= array("type"=>"confirm","name"=>"button","value"=>"删除","_width"=>"500","_height"=>"200","_title"=>'是否确定删除该记录?',"_url"=>"/Mall/StoBanner/delStoBanner?ids=".Encode($row['id']));
    ...
    */
 	$search = array(
        "type"=>array("type"=>"select","name"=>"类型", "value"=>"1", 'option' =>  ['1' => '头部', '2'=>'中间', '3'=>'底部'])
    );

    //自动生成按钮
    $button = array(
            "bt1"=>array("type"=>"popup_listpage","name"=>"button","value"=>"添加广告","_width"=>"550","_height"=>"450","_title"=>'添加广告',"_url"=>"/Mall/StoBanner/addStoBanner"),

            "bt2"=>array("type"=>"confirm_all","name"=>"button","value"=>"批量删除","_width"=>"500","_height"=>"200","_title"=>'删除选中项?',"_url"=>"/Mall/StoBanner/delStoBanner?rel=delete")
        );
