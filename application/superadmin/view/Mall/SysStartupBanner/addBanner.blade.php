...
<?php 
    ...
    */
    ...
    $field = array(
            "bname"=>array("type"=>"text","name"=>"banner名称","validator"=>"required:true", "value"=>$bannerData['bname']),
            "thumb"=>array("type"=>"imgupload", "name"=>"图片地址","validator"=>"required:true", "value"=>$bannerData['thumb']),
            "urltype"=>array("type"=>"select", "name"=>"跳转类型", 'option' => $skipurl,"validator"=>"required:true", "value"=>$bannerData['urltype']),
            "url"=>array("type"=>"text","name"=>"跳转链接","validator"=>"required:true", "value"=>$bannerData['url']),
            "enable"=>array("type"=>"select","name"=>"启用状态","validator"=>"required:true", "value"=>$bannerData['enable'],"option"=>['-1'=>"禁用","1"=>"启用"]),
            "banner_type"=>array("type"=>"select","name"=>"启动类型","validator"=>"required:true", "value"=>$bannerData['banner_type'],"option"=>['1'=>"打开就启动","2"=>"启动一次"]),
            "time"=>array("type"=>"text","name"=>"广告显示时间(s)","validator"=>"required:true", "value"=>$bannerData['time']),
            "sort"=>array("type"=>"text","name"=>"排序","validator"=>"required:true", "value"=>$bannerData['sort']),
            "id"=>array("type"=>"hidden","value"=>$bannerData['id']),
            "submit"=>array("type"=>"submit","name"=>"提交"), //加该参数表示独立页面，不加该参数表示弹窗
        );
