...
//             $list['list'][$k]['returnstatus'] = Model::ins("OrdOrderInfo")->getRow(["orderno" => $v['orderno']], "return_status")['return_status'];
    ...
    public function exportAction(){
        ...
        */
        ...
        // if(!empty($orderno_str))
            $item_list = $OrdOrderItem->getList(['orderid'=>['in',$orderno_str]],'orderno,productname,skuid,productid,productnum,prouctprice,bullamount','id desc',$count,$offset*$count);
            ...
            foreach ($item_list as $key => $value) {
                ...
                $item_list[$key]['leavemessage'] = $Logistics['leavemessage'];
    ...
    */
    public function productlistAction() {
        ...
        $list = Model::ins("OrdOrderItem")->pageList($where, "*", "addtime desc,orderno asc");

        $orderOBJ = Model::ins("OrdOrder");
        
        foreach ($list['list'] as $k => $v) {

            $orderinfo = $orderOBJ->getRow(["orderno"=>$v['orderno']],"id,cust_name,orderstatus");

            $list['list'][$k]['cust_name'] = $orderinfo['cust_name'];
            $list['list'][$k]['orderstatus'] = $orderinfo['orderstatus'];
            $list['list'][$k]['orderid'] = $orderinfo['id'];
...
//             $list['list'][$k]['returnstatus'] = Model::ins("OrdOrderInfo")->getRow(["orderno" => $v['orderno']], "return_status")['return_status'];
            ...
            $list['list'][$k]['thumb'] = Img::url($v['thumb'],60,60);
        }
...
//             "orderpay"=>$orderpay,
    ...
    */
    public function lookLogisticsInfoAction() {
        ...
        // 订单收货信息
        $logistics = Model::ins("OrdOrderLogistics")->getRow(array("orderno" => $order['orderno']), "express_name, express_no");
    ...
    */
    public function setExpressAction() {
        ...
        $order = Model::ins("OrdOrder")->getRow(array("orderno" => $this->getParam('orderno'), "businessid" => $this->businessid));
        if(empty($order)) {
            $this->showError("您无权限访问此处");
        }
        // 识别订单是否已经被申请退款
...
//         }
    ...
    */
    public function updateExpressInfoAction() {
        ...
        // 查看该订单信息
        $orderLogistics = Model::ins("OrdOrderLogistics")->getRow(["id"=>$where["id"]],"orderno");
        
        $order = Model::ins("OrdOrder")->getRow(["orderno"=>$orderLogistics['orderno'],"businessid"=>$where['businessid']],"id");
...
//         $order = Model::ins("OrdOrder")->getRow($where);
    ...
    public function updateExpressAction() {
        ...
        if($order['businessid'] != $this->businessid) {
            $this->showError("您无权限访问此处");
        }
    ...
    */
    public function giftOrderListAction() {
        if(!CommonModel::getVipBusiness($this->businessid)) {
            $this->showError("您无权操作");
        }
        $where = array();
...
//             $orderList['list'][$k]['logisticsAddress'] = $orderLogistics['address'];
    ...
    */
    public function setGiftExpressAction() {
        ...
        if($roleOrder['businessid'] != $this->businessid) {
    ...
    */
    public function updateGiftExpressAction() {
        ...
        Model::ins("RoleOrderLogistics")->modify($logistics, array("id" => $this->getParam("id")));
    ...
    */
    public function lookGiftLogisticsAction() {
        ...
        // 查看订单状态
        $orderInfo = Model::ins("RoleOrder")->getRow(array("id" => $this->getParam("id")), "orderstatus, businessid");

        if($orderInfo['businessid'] != $this->businessid) {
            $this->showError("无访问权限");
        }
    ...
    public function exportproductlistAction(){
        ...
        if(!empty($ids))
            $where = ['id'=>['in',$ids]];
        $orderList = Model::ins("OrdOrderItem")->getList($where, "orderno,addtime,productname,productnum,prouctprice,bullamount,skuid,businessid,productid,remark", "addtime desc,orderno asc",$count,$offset*$count);
        $orderstatus_arr = [
            '0'=>'待付款',
            '1'=>'已付款待发货',
            '2'=>'已发货',
        ...
        foreach ($orderList as $k => $v) {
            ...
            $order = Model::ins('OrdOrder')->getRow(['orderno'=>$v['orderno']],'isabroad,orderstatus,cust_name,productamount,totalamount,bullamount as totalbullamount,actualfreight');  
            $abroad_arr = [
                '0' => '国内订单',
                '1' => '国外订单'
            ];
            $orderList[$k]['isabroad'] = $abroad_arr[$order['isabroad']];
            ...
            $logistics = Model::ins('OrdOrderLogistics')->getRow(['orderno'=>$v['orderno']],'realname,mobile,idnumber,city,address,leavemessage');
            $orderList[$k]['realname'] = $logistics['realname'];
            $orderList[$k]['mobile'] = $logistics['mobile'];
            $orderList[$k]['idnumber'] = $logistics['idnumber'];
            $orderList[$k]['address'] = $logistics['city'] . $logistics['address'];
            $orderList[$k]['leavemessage'] = $v['remark'];
            unset($orderList[$k]['remark']);
        }
        ...
        */
        echo $this->iniExcelData(array(
                            "filename"=>"商品订单导出列表",
                            "head"=>array("订单号","下单时间","商品名称","购买数量","商品价格","牛豆数量","商品属性","订单类型","订单状态","商品货号","商品条形码","商家名称","商家电话","买家名称","商品总额","订单总额","订单总牛豆数","运费","收货人姓名","收货人电话","身份证号码","收货地址","留言"), //excel表头
                            "list"=>$orderList,
                            "offset"=>$offset,
                        ));
        exit;
