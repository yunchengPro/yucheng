...
class Html{
    ...
    */
    ...
    /**
     * [selectpicker selectpicker方法]
     * @Author   ISir<673638498@qq.com>
     * @DateTime 2017-05-17T12:23:02+0800
     * @param    [type]                   $param [description]
     * @return   [type]                          [description]
     */
    public function selectpicker($param){
        $html.="<select";
        $html.=" id=\"".($param['id']!=''?$param['id']:$param['name'])."\"";
        $html.=" name=\"".$param['name']."\"";

        $html.=" data-live-search='true'  class=\"selectpicker select width250 ".($param['class']!=''?$param['class']:"")."\"";
    ...
    */
    public static function category($param){
        ...
        $class      = $param['class']!=''?$param['class']:"input";
        $topcate   = $param['name']."_top";
        $soncate    = $param['name']."_son";
        
        $pvalue  =  empty($param['pvalue'])  ? ""  : $param['pvalue']; 
        $svalue  =  empty($param['svalue'])  ? ""  : $param['svalue'];

        $class      = $param['class']!=''?$param['class']:"select width250";

        $html.="<select name=\"".$topcate."\" id=\"".$topcate."\" class=\"".$class."\" ".$param['other'].">";
        $html.="</select>";
        $html.="<select name=\"".$soncate."\" id=\"".$soncate."\" class=\"".$class."\" ".$param['other'].">";
        $html.="</select>";
        $html.="<script type=\"text/javascript\">";
        $html.='$(document).ready(function(){';
        if($param['hasbull'] == 0){
            $html.="inicategoryno('".$topcate."','".$soncate."','".$pvalue."','".$svalue."');";
        }else{
            $html.="inicategory('".$topcate."','".$soncate."','".$pvalue."','".$svalue."');";
        }
        $html.='});';
        $html.="</script>";
        return $html;
    ...
    */
    ...
    public static function imgupload($param){
        if($param['id']!=''){
            ...
            $html .= '<span class="btn btn-danger fileinput-button" id="'.$param['id'].'_upload" name="'.$param['id'].'_upload">';
            ...
            if(!empty($param['value'])){
                ...
                foreach($value_arr as $value){
                    ...
                    $html.='<div>';
                    $url = Img::url($value, 100, 100,'',false);
                    $gourl = Img::url($value);
                    $html.='<img src="'.$url.'">';
                    $html.='</div>';
                    ...
                    $html.= '<input type="text" style="width:20px;margin-left:5px;"" class="'.$param['id'].'_sort" src="'.$value.'" onChange="fileupload_changesort(\''.$param['id'].'\')" value="'.$i.'">';
 ...
 * [cubstr 截取字符不会有乱码]
 * @Author   ISir<673638498@qq.com>
 * @DateTime 2017-04-11T14:39:25+0800
 * @param    [type]                   $string     [需要截取字符]
 * @param    [type]                   $beginIndex [起始位置]
 * @param    [type]                   $length     [取长度]
 * @return   [type]                               [description]
 */
