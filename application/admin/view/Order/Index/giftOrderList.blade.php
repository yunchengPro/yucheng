<?php 
    ...
    foreach ($pagelist as $key => $row) {
        if($row['orderstatus'] == 1) 
            $pagelist[$key]['act'][] = array("type" => "popup_listpage", "name" => "button", "value" => "发货", "_width" => "550", "_height" => "300", "_title" => "发货", "_url" => "/Order/Index/setGiftExpress?id=".Encode($row['id']));
        
        if($row['orderstatus'] == 2)
            $pagelist[$key]['act'][] = array("type" => "popup_listpage", "name" => "button", "value" => "修改物流信息", "_width" => "650", "_height" => "550", "_title" => "修改快递信息", "_url" => "/Order/Index/setGiftExpress?id=".Encode($row['id']));
        
        if($row['orderstatus'] != 0 && $row['orderstatus'] != 1)
            $pagelist[$key]['act'][] = array("type" => "popup_listpage", "name" => "button", "value" => "查看物流信息", "_width" => "1000", "_height" => "600", "_title" => "查看快递信息", "_url" => "/Order/Index/lookGiftLogistics?id=".Encode($row['id']));
