{include file="Pub/header" /}
	<header class="page-header">
		<div class="page-bar">
			...
			<div class="bar-title">我要申请成为区代理</div>
		</div>
	</header>
	...
	<section class="register-role">
		...
		<div class="register-item">
			...
			<input type="text" placeholder="区代理所在的省市区" id="txt_area" class="area_inp" > 
				<input type="hidden" id="hd_area" value="110000, 110100, 110101" > <i></i>
		</div>
	...
	<div class="register-role-oper">
		<button v-on:click="applyToManager()">提交</button>
	</div>
...
<script type="text/javascript">
	var vm=	new Vue({
			el: '#app',
			data: {
			   	apiUrl:'/User/Apply/doapplyToManager',	
			   	name:'',
			   	// mobile:'',
			   	address:'',
			   	join_type:1,
		  	...
		  	methods:{
		  		...
		  		closeMask:function(){
		  			this.maskShow=false;
		  			window.location.href = '/user/apply/applymanprogress?orderno='+this.orderno;
		  		},
			  	applyToManager:function(){
		  			var _this=this;
	  			  	loadtip({
                        content:'加载中..'
                    });
                    _this.$http.post(_this.apiUrl,{
                        ...
                        function (res) {
                            ...
                            if(data.code=='200'){
                            	...
                            	//_this.sureOrder();
                            	window.location.href = '/user/apply/applymanprogress?orderno='+_this.orderno;
                                
                           	}else{
                           		toast(data.msg);
                           		_this.orderno = data.data.orderno;
                           		//alert(_this.orderno);
                           		if(data.code==30018 && _this.orderno != '')
                           			window.location.href = '/user/apply/applymanprogress?orderno='+_this.orderno;
                           	}
                            //$("#city").val('');
                        },function (res) {
